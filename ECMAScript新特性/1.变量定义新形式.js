...
/*
    深入理解const在js引擎中的实现
    const 具有与 let 相同的底层实现原理，区别在于 const 定义的变量被视为常量（在赋值之后无法更改），
    因此变量声明时 必须初始化。此外，应该注意的是，使用 const 声明的对象是可以修改属性的。
    在定义 const 对象时，对象本身是常量，而不是对象的属性。
    只有对象本身不能被修改，而对象包含的属性可以任意修改。
*/

/*
    暂时性死区就是子屏蔽环境 和 webcomponent技术中的shadowDOM一样
*/

/*
    面试的时候
    一般使用var进行埋坑 让你去进行排坑 最后让你去进行排坑
*/